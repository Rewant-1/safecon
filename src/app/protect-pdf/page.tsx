...
export default function ProtectPdfPage() {
  ...
  const handleProcess = async () => {
    ...
    await process(async () => {
      return await protectPdf(file, { userPassword: password });
    });
  };
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = file?.name.replace(/\.pdf$/i, "_protected.pdf") ?? "protected.pdf";
    a.click();
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="mb-16 text-center">
        ...
        <h1 className="text-4xl sm:text-5xl font-bold tracking-tight text-black mb-4">Protect PDF</h1>
        <p className="text-muted text-lg font-light">Encrypt your document with a password. Ironclad privacy.</p>
      </div>
      ...
      {status === "idle" && !file && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <FileDropzone onFilesAccepted={handleFiles} accept={{ "application/pdf": [".pdf"] }} maxFiles={1} label="Drop your PDF here" sublabel="We'll encrypt it with AES" />
        </motion.div>
      )}
      ...
      {status === "idle" && file && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
          <div className="flex items-center justify-between p-5 rounded-[24px] bg-white border border-black/5 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
            ...
            <button onClick={handleReset} aria-label="Reset file" className="text-black/20 hover:text-black transition-colors cursor-pointer"><Lock className="w-4 h-4" /></button>
          </div>

          <div className="p-6 rounded-2xl bg-white border border-black/5 shadow-[0_8px_30px_rgb(0,0,0,0.03)]">
            <label htmlFor="protect-password" className="block text-xs font-bold tracking-widest text-black/40 uppercase mb-3">Password</label>
            <div className="relative">
              <input id="protect-password" type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter a strong password"
            ...
            <p className="text-[10px] text-black/30 tracking-wider uppercase mt-3">Encryption happens entirely in your browser. We never see your password.</p>
          </div>
      ...
      {status === "done" && result && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center py-16 px-4 rounded-[32px] bg-white border border-black/5 shadow-[0_20px_60px_rgb(0,0,0,0.06)]">
          <div className="w-20 h-20 rounded-[24px] bg-emerald-50 text-emerald-500 flex items-center justify-center mb-6"><Lock className="w-8 h-8" strokeWidth={1.5} /></div>
          <h2 className="text-3xl font-bold tracking-tight mb-2">Fortified</h2>
          <p className="text-black/40 mb-8">Your document is now password-protected</p>
